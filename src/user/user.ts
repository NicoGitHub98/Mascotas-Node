...
UserSchema.methods.follow = function (user_id: Schema.Types.ObjectId) {
  this.following.push(user_id);

  this.updated = Date.now();
};

UserSchema.methods.unfollow = function (user_id: Schema.Types.ObjectId) {
  var index = this.following.indexOf(user_id);
  if (index > -1) {
